...
# Data
dat <- read.table("gss18cat.raw",header=T)
...
#
 ww <- matrix(NA,njobs,nw)
 for( i in 1:njobs )
  {
   ind <- (1:nworkers)[ dat$occ17==(i-1) ]
   ww[i,1] <- unique( dat$presmean[ind] )
   ww[i,2] <- unique( dat$autmean[ind] )
   }
...
alpha <- rep(0,nw)            # worker preferences
...
# get 1-sided logit estimates for starting values
for( j in 2:njobs )
 {
  y <- as.numeric( opp[,j] )
